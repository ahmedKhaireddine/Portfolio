<template>
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="25px" height="25px" fill="currentColor">
		<path
			style="
				text-indent: 0;
				text-align: start;
				line-height: normal;
				text-transform: none;
				block-progression: tb;
				-inkscape-font-specification: Bitstream Vera Sans;
			"
			d="M 2 6 L 2 7 L 2 25 L 2 26 L 3 26 L 7 26 L 7 29 L 7 31.09375 L 8.625 29.78125 L 13.34375 26 L 29 26 L 30 26 L 30 25 L 30 7 L 30 6 L 29 6 L 3 6 L 2 6 z M 4 8 L 28 8 L 28 24 L 13 24 L 12.65625 24 L 12.375 24.21875 L 9 26.90625 L 9 25 L 9 24 L 8 24 L 4 24 L 4 8 z M 15 10 L 15 12 L 17 12 L 17 10 L 15 10 z M 15 14 L 15 22 L 17 22 L 17 14 L 15 14 z"
			overflow="visible"
			font-family="Bitstream Vera Sans"
		/>
	</svg>
</template>
